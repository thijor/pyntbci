...
for subject in subjects:
    ...
    if not os.path.exists(os.path.join(path, "derivatives", "offline", subject)):
        os.makedirs(os.path.join(path, "derivatives", "offline", subject))

    # Save data
    np.savez(os.path.join(path, "derivatives", "offline", subject, f"{subject}_gdf.npz"), X=X, y=y, V=V, fs=fs)
